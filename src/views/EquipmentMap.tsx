...
import { Equipment, EquipmentStateHistory, EquipmentPositionHistory, EquipmentStatus, EquipmentState } from '../models/EquipmentModel';
...
const MapController = ({ 
    ...
    useEffect(() => {
        if (shouldFly) {
            if (offsetX !== 0) {
                const targetPoint = map.project(selectedPosition, zoom).add([offsetX, 0]);
...
export const EquipmentMap: React.FC = () => {
    ...
    const [stateHistory, setStateHistory] = useState<{ [key: string]: EquipmentStateHistory }>({});
    const [positionHistory, setPositionHistory] = useState<{ [key: string]: EquipmentPositionHistory }>({});
    const [selectedEquipment, setSelectedEquipment] = useState<string | null>(null);
    const [showRoute, setShowRoute] = useState(false);
    const [mapCenter, setMapCenter] = useState<[number, number]>([-19.126536, -45.947756]);
    ...
    useEffect(() => {
        const initializeData = async () => {
            ...
            for (const equipment of allEquipments) {
                const equipmentStateHistory = await equipmentController.getEquipmentStateHistory(equipment.id);
                const equipmentPositionHistory = await equipmentController.getEquipmentPositionHistory(equipment.id);
    ...
    const getRoutePoints = (equipmentId: string, upToIndex?: number): [number, number][] => {
        ...
        if (typeof upToIndex === 'number') {
            return sortedPositions.slice(0, upToIndex + 1).map(pos => [pos.lat, pos.lon]);
        }
    ...
    const RouteControl = () => {
        ...
        if (!history || !history.positions || history.positions.length === 0) return null;
    ...
    useEffect(() => {
        ...
        const history = positionHistory[selectedEquipment];
        if (!history || !history.positions || history.positions.length === 0) return;
    ...
    const filteredEquipments = equipments.filter(equipment => {
        ...
        const matchesSearch = searchQuery === '' || 
            ...
            (currentState?.name.toLowerCase().includes(searchLower) || false);
                    ...
                    <Polyline
                        ...
                        color={getEquipmentTypeColor(
                            equipmentController.getEquipmentModelById(
                                equipments.find(e => e.id === selectedEquipment)?.equipmentModelId || ''
                            )?.name
                        )}
                        weight={2.5}
                        opacity={0.7}
                            ...
                            <Popup>
                                <div className="map-popup">
                                    <div className="popup-header">
                                        <i className={`fas ${getEquipmentTypeIcon(model?.name)}`} style={{ color: getEquipmentTypeColor(model?.name) }}></i>
                                        <h3>{equipment.name}</h3>
                                    </div>
                                    <div className="popup-model">{model?.name}</div>
                                    
                                    <div className="popup-state" style={{ backgroundColor: `${state?.color}20` }}>
                                        <i className="fas fa-circle" style={{ color: state?.color }}></i>
                                        <span style={{ color: state?.color }}>{state?.name || 'Desconhecido'}</span>
                                    </div>
                                    ...
                                    <div className="popup-stats">
                                        <div className="popup-stat">
                                            ...
                                            <div className="popup-stat-value">
                                                <i className="fas fa-chart-line" style={{ marginRight: '4px', color: '#2ecc71' }}></i>
                                        ...
                                        <div className="popup-stat">
                                            ...
                                            <div className="popup-stat-value">
                                                <i className="fas fa-dollar-sign" style={{ marginRight: '4px', color: '#3498db' }}></i>
                                    ...
                                    {history && history.states && history.states.length > 0 && (
                                        <div className="popup-history">
                                            ...
                                            <div className="history-list">
                                                {history.states.slice().reverse().map((historyState, index) => {
                                                    const historyStateDetails = equipmentController.getEquipmentStateById(historyState.equipmentStateId);
                                                    return (
                                                        <div key={index} className="history-item">
                                                            <div className="history-state">
                                                                <i className="fas fa-circle" style={{ color: historyStateDetails?.color }}></i>
                                                                <span>{historyStateDetails?.name || 'Desconhecido'}</span>
                                                            </div>
